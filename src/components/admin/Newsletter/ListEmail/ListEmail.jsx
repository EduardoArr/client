  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setEmails(response.emailStored.docs);
        setPagination({
          limit: response.emailStored.limit,
          page: response.emailStored.page,
          pages: response.emailStored.pages,
          total: response.emailStored.total,
        });
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <div className="list-emails">
      ...
      <div className="list-emails__pagination">
        <Pagination totalPages={pagination.pages} defaultActivePage={pagination.page} ellipsisItem={false} firstItem={false} lastItem={false} onPageChange={changePage} />
      </div>
    </div>
  );
};
