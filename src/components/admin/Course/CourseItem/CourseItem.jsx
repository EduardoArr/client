...
export const CourseItem = (props) => {
  ...
  const onDelete = async () => {
    try {
      await courseController.deleteCourses(accessToken, course._id);
      onReload();
      onOpenCloseConfirm();
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      <Confirm open={showConfirm} onCancel={onOpenCloseConfirm} onConfirm={onDelete} content={`Eliminar el curso ${course.title}`} size="mini"></Confirm>
    </>
  );
};
