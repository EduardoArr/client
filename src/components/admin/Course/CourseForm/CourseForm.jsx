...
export const CourseForm = (props) => {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    //Esto es para mostrar la imagen en el drag and drop
    formik.setFieldValue("miniature", URL.createObjectURL(file));
    //Esto para subirlo al server
    formik.setFieldValue("file", file);
  });

  //el getroot props saca todas las propiedades del archivo que arrastramos al drop y lo agregamos al div con {...getRootProps()}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) => {
      try {
        if (!course) {
          await courseController.createCourses(accessToken, formValue);
        } else {
          await courseController.updateCourses(accessToken, course._id, formValue);
        }
        onReload();
        onClose();
      } catch (error) {
  ...
  return (
    <Form className="course-form" onSubmit={formik.handleSubmit}>
      ...
      <Form.TextArea name="description" placeholder="Descripción del curso" onChange={formik.handleChange} value={formik.values.description} error={formik.errors.description} />
